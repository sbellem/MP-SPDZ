...
import math
from mpspdz import floatingpoint
from mpspdz import types
from mpspdz import comparison
from mpspdz import program
from mpspdz import instructions_base
...
p_3508 = [
    ...
    0.04166666666666667129,
    -0.00138888888888888873,
    0.00002480158730158702,
    -0.00000027557319223933,
    0.00000000208767569817,
    -0.00000000001147074513,
    0.00000000000004779454,
    -0.00000000000000015612,
    0.00000000000000000040,
]
##
# @private
p_1045 = [
    1.000000077443021686,
    0.693147180426163827795756,
    0.224022651071017064605384,
    0.055504068620466379157744,
    0.009618341225880462374977,
    0.001332730359281437819329,
    0.000155107460590052573978,
    0.000014197847399765606711,
...
##
# @private
q_2524 = [+0.353553425277, +4.54517087629, +6.42784209029, +1]
##
# @private
p_5102 = [
    +21514.05962602441933193254468,
    +73597.43380288444240814980706,
    +100272.5618306302784970511863,
    +69439.29750032252337059765503,
    +25858.09739719099025716567793,
    +5038.63918550126655793779119,
    +460.1588804635351471161727227,
    +15.08767735870030987717455528,
    +0.07523052818757628444510729539,
]
##
# @private
q_5102 = [
    +21514.05962602441933193298234,
    +80768.78701155924885176713209,
    +122892.6789092784776298743322,
    +97323.20349053555680260434387,
    +42868.57652046408093184006664,
    +10401.13491566890057005103878,
    +1289.75056911611097141145955,
    +68.51937831018968013114024294,
    +1,
]
##
# @private
p_4737 = [
    -9338.550897341021522505385079,
    +43722.68009378241623148489754,
    -86008.12066370804865047446067,
    +92190.57592175496843898184959,
    -58360.27724533928122075635101,
    +22081.61324178027161353562222,
    -4805.541226761699661564427739,
    +542.2148323255220943742314911,
    -24.94928894422502466205102672,
    0.2222361619461131578797029272,
]
##
# @private
q_4737 = [
    -9338.550897341021522505384935,
    +45279.10524333925315190231067,
    -92854.24688696401422824346529,
    +104687.2504366298224257408682,
    -70581.74909396877350961227976,
    +28972.22947326672977624954443,
    -7044.002024719172700685571406,
    +935.7104153502806086331621628,
    -56.83369358538071475796209327,
    1,
]
##
# @private
...
def exp2_fx(a):
    ...
    if types.program.options.ring:
        ...
        lower_overflow = comparison.CarryOutRaw(
            masked_bits[a.f - 1 :: -1], r_bits[a.f - 1 :: -1]
        )
        ...
        higher_bits = r_bits[0].bit_adder(
            r_bits[a.f : n_bits],
        ...
        carry = comparison.CarryOutLE(
            masked_bits[n_bits:-1], r_bits[n_bits:-1], higher_bits[-1]
        )
        # should be for free
        highest_bits = r_bits[0].ripple_carry_adder(
            masked_bits[n_bits:-1], [0] * (a.k - n_bits), carry_in=higher_bits[-1]
        )
        bits_to_check = [
            x.bit_xor(y) for x, y in zip(highest_bits[:-1], r_bits[n_bits:-1])
        ]
        t = sint.conv(floatingpoint.KOpL(lambda x, y: x.bit_and(y), bits_to_check))
        # sign
...
def lin_app_SQ(b, k, f):
    ...
    # c is now escalated
    w = (
        alpha * load_sint(c, types.sfix) + beta
    )  # equation before b and reduction by order of k
