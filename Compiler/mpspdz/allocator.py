...
from mpspdz.exceptions import CompilerError, RegisterOverflowError

# from mpspdz.config import *
...
from mpspdz.instructions import (
    ...
    popint_class,
    pushint_class,
    startprivateoutput_class,
    stopprivateoutput_class,
)
...
from mpspdz.instructions_base import (
    ...
    DoNotEliminateInstruction,
    IndirectMemoryInstruction,
    IntegerInstruction,
    IOInstruction,
    PublicFileIOInstruction,
    RawInputInstruction,
...
from mpspdz.util import defaultdict_by_id, dict_by_id, format_trace, set_by_id
import mpspdz.graph
import mpspdz.program
...
class StraightlineAllocator:
    ...
    def __init__(self, n, program):
        ...
        self.usage = mpspdz.program.RegType.create_dict(lambda: 0)
    ...
    def process(self, program, alloc_pool):
            ...
            for j in i.get_def():
                if j.vectorbase in self.alloc:
                    if j in self.defined:
                        raise CompilerError(
                            "Double write on register %s "
                            "assigned by '%s' in %s" % (j, i, format_trace(i.caller))
                        )
                else:
                    # unused register
        ...
        #     (self.usage[mpspdz.program.RegType.ClearModp], self.usage[mpspdz.program.RegType.SecretModp])
        # print "GF2N usage: %d clear, %d secret" % \
        #     (self.usage[mpspdz.program.RegType.ClearGF2N], self.usage[mpspdz.program.RegType.SecretGF2N])
        return self.usage
...
class Merger:
    ...
    def longest_paths_merge(self):
        ...
        last_nodes = [None, None]  # noqa F841
        ...
        order = mpspdz.graph.topological_sort(G, preorder)
        instructions[:] = [
            instructions[i] for i in order if instructions[i] is not None
        ]
        if len(instructions) > 100000:
    ...
    def dependency_graph(self, merge_classes):  # noqa C901
        """ Create the program dependency graph. """
        block = self.block
        options = self.options
        open_nodes = set()
        self.open_nodes = open_nodes
        colordict = defaultdict(  # noqa F841
            lambda: "gray",
            asm_open="red",
            ldi="lightblue",
            ldm="lightblue",
        ...
        G = mpspdz.graph.SparseDiGraph(len(block.instructions))
        self.G = G
        ...
        def handle_mem_access(
            addr, reg_type, last_access_this_kind, last_access_other_kind
        ):
        ...
        def mem_access(n, instr, last_access_this_kind, last_access_other_kind):
            ...
            if isinstance(addr, int):
                for i in range(min(instr.get_size(), 100)):
                    addr_i = addr + i
                    handle_mem_access(
                        addr_i, reg_type, last_access_this_kind, last_access_other_kind
                    )
                if not warned_about_mem and (instr.get_size() > 100):
                    print(
                        "WARNING: Order of memory instructions "
        ...
        def keep_merged_order(instr, n, t):
            if last_input[t][0] is not None:
                if instr.merge_id() != block.instructions[last_input[t][0]].merge_id():
        ...
        for n, instr in enumerate(block.instructions):
            ...
            if isinstance(instr, merge_classes):
                ...
                skipped_depths = set()
                while (
                    depth in round_type and round_type[depth] != instr.merge_id()
                ) or (
                    int(options.max_parallel_open) > 0
                    and parallel_open[depth] >= int(options.max_parallel_open)
                ):
                    skipped_depths.add(depth)
                    depth = next_available_depth.get((type(instr), depth), depth + 1)
            ...
            if isinstance(instr, ReadMemoryInstruction):
                if options.preserve_mem_order:
                    if (
                        last_mem_write
                        and last_mem_read
                        and last_mem_write[-1] > last_mem_read[-1]
                    ):
                        last_mem_read[:] = []
            ...
            if n % 100000 == 0 and n > 0:
                print(
                    "Processed dependency of %d/%d instructions at"
        ...
        if len(open_nodes) > 1000 and self.block.parent.program.verbose:
            print(
                "Basic block has %d %s instructions" % (len(open_nodes), merge_classes)
            )
        ...
        G.add_edges_from(
            list(
                zip(
                    G.pred[j],
                    itertools.cycle([i]),
                    [G.weights[(k, j)] for k in G.pred[j]],
                )
            )
        )
    ...
    def eliminate_dead_code(self):
        ...
        for i, inst in zip(
            range(len(instructions) - 1, -1, -1), reversed(instructions)
        ):
            if inst is None:
                continue
            ...
            if (
                isinstance(inst, popint_class)  # noqa F405
                and (
                    not G.degree(i)
                    or (
                        G.degree(i) == 1
                        and isinstance(instructions[list(G[i])[0]], StackInstruction)
                    )
                )
                and inst.args[0].can_eliminate
                and len(G.pred[i]) == 1
                and isinstance(instructions[list(G.pred[i])[0]], pushint_class)
            ):
        ...
        if count > 0 and self.block.parent.program.verbose:
            print(
                "Eliminated %d dead instructions, among which %d opens: %s"
...
class RegintOptimizer:
    def __init__(self):
        self.cache = dict_by_id()

    def run(self, instructions):
        for i, inst in enumerate(instructions):
            if isinstance(inst, ldint_class):  # noqa F405
                ...
                if inst.args[1] in self.cache and inst.args[2] in self.cache:
                    res = inst.op(self.cache[inst.args[1]], self.cache[inst.args[2]])
                    if abs(res) < 2 ** 31:
                        self.cache[inst.args[0]] = res
                        instructions[i] = ldint(inst.args[0], res, add_to_prog=False)
                elif isinstance(inst, addint_class):  # noqa F405
                    if inst.args[1] in self.cache and self.cache[inst.args[1]] == 0:
                        instructions[i] = inst.args[0].link(inst.args[2])
                    elif inst.args[2] in self.cache and self.cache[inst.args[2]] == 0:
            ...
            elif isinstance(inst, convint_class):  # noqa F405
                if inst.args[1] in self.cache:
                    res = self.cache[inst.args[1]]
                    self.cache[inst.args[0]] = res
                    if abs(res) < 2 ** 31:
                        instructions[i] = ldi(inst.args[0], res, add_to_prog=False)
            elif isinstance(inst, mulm_class):  # noqa F405
                if inst.args[2] in self.cache:
                    op = self.cache[inst.args[2]]
                    if op == 0:
                        instructions[i] = ldsi(inst.args[0], 0, add_to_prog=False)
                    elif op == 1:
                        instructions[i] = None
