...
from mpspdz.GC.types import *
from mpspdz.library import function_block
...
class Circuit:
    ...
    def run(self, *inputs):
        ...
        if n not in self.functions:
            self.functions[n] = function_block(lambda *args: self.compile(*args))
...
class ieee_float:
    ...
    def __init__(self, value):
        if isinstance(value, sbitvec):
            ...
            self.value = sbitvec(
                sbits.get_type(64)(struct.unpack("Q", struct.pack("d", value))[0])
            )
        else:
    ...
    def __eq__(self, other):
        res = sbitvec.from_vec(self.circuit("eq")(self.value, other.value).v[:1])
        if res.v[0].n == 1:
    ...
    def reveal(self):
        ...
        s = [self.value.v[63]]
        m, e, s = [sbitvec.from_vec(x).elements()[0].reveal() for x in (m, e, s)]
        return cbitfloat(
            2 ** 52 + m,
            e - 2 ** 10 - 51,
            cbit((m.to_regint() == 0) * (e.to_regint() == 0)),
            s,
            (e.to_regint() == 2 ** 11 - 1),
        )
