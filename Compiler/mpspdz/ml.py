...
class DenseBase(Layer):
    ...
    def backward_params(self, f_schur_Y, batch):
        ...
        self.nabla_b.assign(
            sum(sum(f_schur_Y[k][j][i] for k in range(N)) for j in range(self.d))
...
class Dense(DenseBase):
    ...
    def forward(self, batch=None):
        ...
        self.Y.assign_vector(self.f(self.f_input.get_part_vector(0, len(batch))))
...
class Optimizer:
    ...
    def forward(self, N=None, batch=None, keep_intermediate=True, model_from=None):
        """ Compute graph.
...
class SGD(Optimizer):
    ...
    def update(self, i_epoch):
        for nabla, theta, delta_theta in zip(
            self.nablas, self.thetas, self.delta_thetas
        ):
                ...
                if self.debug:
                    for x, name in (
                        ...
                        print_ln_if(
                            (x > 1000) + (x < -1000),
                            name + ": %s %s %s %s",
                            *[y.v.reveal() for y in (old, red_old, new, diff)]
                        )
            ...
            if self.debug:
                ...
                def _(i):
                    x = a[i]
                    print_ln_if((x > 1000) + (x < -1000), "update len=%d" % len(nabla))
                ...
                def _(i):
                    x = a[i]
                    print_ln_if((x > 1000) + (x < -1000), "nabla len=%d" % len(nabla))
