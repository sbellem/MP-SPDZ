if "_Array" not in dir():
    ...
    from mpspdz import permutation

    _Array = Array

from mpspdz import oram
...
class PathORAM(TreeORAM):
    def __init__(
        ...
        self.buckets = RAM(
            self.bucket_size * 2 ** (self.D + 1), self.entry_type, self.get_array
        )
        if init_rounds != -1:
        ...
        if stash_size is None:
            if self.deterministic_eviction:
                if self.bucket_size == 2:
                    ...
                    raise CompilerError(
                        "Bucket size %d not supported" % self.bucket_size
                    )
            else:
                tau = 3
                sigma = 5
        ...
        self.stash = TrivialORAM(
            stash_size, *self.internal_value_type(), index_size=self.index_size
        )
        ...
        self.read_value = [
            Array(self.D + 2, self.value_type.get_type(l)) for l in self.entry_size
        ]
    ...
    def read_and_remove_levels(self, u):
        ...
        for level in range(self.D + 1):
            ...
            for j in range(self.bucket_size):
                found[j] = indices[j].equal(u, self.index_size) * (1 - empty_bits[j])
            ...
            for i, (j, entry, prod_entry) in enumerate(
                zip(ram_indices, entries, prod_entries)
            ):
    ...
    def shuffle_evict(self, leaf):
        ...
        buckets = [
            [
                [self.temp_storage[j]] + levels[j]
                for j in range(self.bucket_size * i, self.bucket_size * (i + 1))
            ]
        ...
        permutation.odd_even_merge_sort(
            merged_entries, lambda a, b: a[0].empty() * (a[-1] - 1 + b[-1]) + 1 - a[-1]
        )
        ...
        permutation.odd_even_merge_sort(
            empty_bits_and_levels, permutation.bitwise_list_comparator
        )
        ...
        for i, entrylev in enumerate(merged_entries):
            ...
            if i < len(empty_bits_and_levels):
                new_level = (
                    empty_bits_and_levels[i][1] - level
                ) * entry.empty() + level
                if evict_debug:
                    print_ln(
                        ...
                        entry.empty().reveal(),
                        empty_bits_and_levels[i][1].reveal(),
                        level.reveal(),
                        new_level.reveal(),
                    )
            else:
    ...
    def evict_block(self, entry, level, leaf):
        ...
        # leaf = self.state.read().reveal()
        lca_lev, cbits = self.compute_lca(
            entry.x[0], leaf, 1 - entry.empty()
        )  # , level + self.sigma)
    ...
    def read_and_remove(self, u):
        ...
        read_value = sum(
            list(map(operator.mul, not_empty, values)),
    ...
    def adjust_lca(self, lca_bits, lev, not_empty, prnt=False):
        ...
        for j in range(upper, lower - 1, -1):
            ...
            if self.bucket_size == 4:
                new_lca[j] = try_add_here * (
                    1 - self.size_bits[j][2]
                )  # (not_empty => lca_bits all 0)
    ...
    def compute_lca(self, a, b, not_empty, limit=None):
        ...
        if self.use_shuffle_evict:
            ...
            lca = (
                sum(found[i] * (one << i) for i in range(self.D)[:limit])
                + (one << limit) * not_found
            )
            return (
                Counter(lca, max_val=limit, value_type=self.value_type),
                found + [not_found],
            )
    ...
    def compute_pos(self, entry, lev, levstar, leaf):
        ...
        stash_eq = Counter(
            self.stash_size.value * add_to_stash, len(self.stash.ram)
        ).equal_range(self.stash.size)
