...
import functools
import copy
from mpspdz.exceptions import ArgumentError, CompilerError
from mpspdz.config import *
from mpspdz import util
from mpspdz import tools
...
def vectorize(instruction, global_dict=None):
    ...
    class Vectorized_Instruction(instruction):
        ...
        def copy(self, size, subs):
            return type(self)(size, *self.get_new_args(size, subs), copying=True)
...
def cisc(function):
    class MergeCISC(Mergeable):
        ...
        def merge_id(self):
            return self.function, tuple(self.params), tuple(sorted(self.kwargs.items()))
        ...
        def new_instructions(self, size, regs):
            if self.merge_id() not in self.instructions:
                ...
                from mpspdz.allocator import Merger

                merger = Merger(block, program.options, tuple(program.to_merge))
                ...
                self.instructions[self.merge_id()] = list(filtered), args, n_rounds
...
def sfix_cisc(function):
    from mpspdz.types import sfix, sint, cfix, copy_doc
...
class RegisterArgFormat(ArgFormat):
    ...
    def check(cls, arg):
        ...
        if arg.program != program.curr_tape:
            raise ArgumentError(
                arg,
                "Register from other tape, trace: %s" % util.format_trace(arg.caller),
            )
        if arg.reg_type != cls.reg_type:
            raise ArgumentError(
                arg,
                "Wrong register type '%s', expected '%s'"
...
class Instruction(object):
    ...
    def check_args(self):
        ...
        for n, (arg, f) in enumerate(zip(self.args, self.arg_format)):
            try:
                ...
                raise CompilerError(
                    'Invalid argument %d "%s" to instruction: %s' % (n, e.arg, self)
                    + "\n"
                    + e.msg
                )
    ...
    def get_used(self):
        ...
        return (
            arg
            for arg, w in zip(self.args, self.arg_format)
            if format_str_is_reg(w) and not format_str_is_writeable(w)
        )
...
class DirectMemoryWriteInstruction(DirectMemoryInstruction, WriteMemoryInstruction):
    __slots__ = []
...
class DataInstruction(Instruction):
    ...
    def add_usage(self, req_node):
        req_node.increment(
            (self.field_type, self.data_type), self.get_size() * self.get_repeat()
        )
...
class ClearShiftInstruction(ClearImmediate):
    ...
    def check_args(self):
        ...
        if self.args[2] > bits:
            raise CompilerError(
                "Shifting by more than %d bits " "not implemented" % bits
            )
        elif self.args[2] < 0:
