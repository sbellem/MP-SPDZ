...
class inputmask(base.Instruction):
    ...
    def add_usage(self, req_node):
        req_node.increment((self.field_type, "input", self.args[1]), self.get_size())
...
class asm_input(base.TextInputInstruction):
    ...
    def add_usage(self, req_node):
        for player in self.args[1::2]:
            req_node.increment((self.field_type, "input", player), self.get_size())
...
class inputfloat(base.TextInputInstruction):
    ...
    def add_usage(self, req_node):
        for player in self.args[5::6]:
            req_node.increment((self.field_type, "input", player), 4 * self.get_size())
...
class inputmixed_base(base.TextInputInstruction):
    ...
    types = {0: (1, 0), 1: (1, 1), 2: (4, 1)}
    type_ids = {"int": 0, "fix": 1, "float": 2}
...
class inputmixed(inputmixed_base):
    ...
    def add_usage(self, req_node):
        for i in self.bases():
            ...
            req_node.increment(
                (self.field_type, "input", player), n_dest * self.get_size()
            )
...
class convmodp(base.Instruction):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if bitlength > 64:
            raise CompilerError(
                "%d-bit conversion requested "
                "but integer registers only have 64 bits" % bitlength
            )
...
class mulrs(base.VarArgsInstruction, base.DataInstruction):
    ...
    def get_used(self):
        return sum((arg.get_all() for arg in self.args[2::4] + self.args[3::4]), [])
...
class conv2ds(base.DataInstruction):
    ...
    def get_repeat(self):
        return self.args[3] * self.args[4] * self.args[7] * self.args[8] * self.args[11]
