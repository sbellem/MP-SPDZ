...
def cond_swap_bit(x, y, b):
    ...
    if isinstance(x, list):
        ...
        return [xi - ti for xi, ti in zip(x, t)], [yi + ti for yi, ti in zip(y, t)]
    else:
        t = (x - y) * b
        return x - t, y + t
...
def random_perm(n):
    ...
    if not Program.prog.options.insecure:
        raise CompilerError(
            "no secure implementation of Waksman permution, "
...
def waksman(a, config, depth=0, start=0, reverse=False):
    ...
    for i in range(n // 2):
        if reverse:
            a0[i], a1[i] = cond_swap_bit(
                a[2 * i], a[2 * i + 1], config[depth][i + n // 2 + start]
            )
        else:
            a0[i], a1[i] = cond_swap_bit(
                a[2 * i], a[2 * i + 1], config[depth][i + start]
            )
    ...
    for i in range(n // 2):
        if reverse:
            a[2 * i], a[2 * i + 1] = cond_swap_bit(
                a0[i], a1[i], config[depth][i + start]
            )
        else:
            a[2 * i], a[2 * i + 1] = cond_swap_bit(
                a0[i], a1[i], config[depth][i + n // 2 + start]
            )
...
def iter_waksman(a, config, reverse=False):
    ...
    def create_round_fn(n, reg_type, inwards):
        ...
        def do_round(size, config_address, a_address, a2_address):
            ...
            def f(k):
                ...
                in1, in2 = (
                    (base + j + j * inwards),
                    (base + j + j * inwards + 1 * inwards + sizeval * outwards),
                )
                out1, out2 = (
                    (base + j + j * outwards),
                    (base + j + j * outwards + 1 * outwards + sizeval * inwards),
                )
    ...
    do_round = lambda size, ca, aa, aa2, inwards: create_round_fn(
        n, a[0].reg_type, inwards
    )(size, ca, aa, aa2)
...
def shuffle_entries(
    x, entry_cls, config=None, value_type=sgf2n, reverse=False, perm_size=None
):
