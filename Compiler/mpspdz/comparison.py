...
def TruncRing(d, a, k, m, signed):
    if program.use_split() == 3:
        ...
        if m == 1:
            low = x[1][1]
            high = sint.conv(CarryOutLE(x[1][:-1], x[0][:-1])) + sint.conv(x[0][-1])
        else:
            mid_carry = CarryOutRawLE(x[1][:m], x[0][:m])
            low = sint.conv(mid_carry) + sint.conv(x[0][m])
            tmp = util.tree_reduce(
                carry,
                (sbitint.half_adder(xx, yy) for xx, yy in zip(x[1][m:-1], x[0][m:-1])),
            )
...
def Mod2mRing(a_prime, a, k, m, signed):
    assert int(program.options.ring) >= k
    from mpspdz.types import sint, intbitint, cint
