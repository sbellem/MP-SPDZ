from mpspdz.path_oram import *
from mpspdz.util import bit_compose
...
def gu_step(high, low):
    ...
    not_greater = high[1]
    return (
        if_else(not_greater, 0, high[0] + low[0]),
...
def comp_step(high, low):
    ...
    return (
        sum(map(operator.mul, indicator, (1, 0, low[0]))),
...
class CircuitORAM(PathORAM):
    def __init__(
        ...
        self.L = self.D + 1
        print(
            "create oram of size %d with depth %d and %d buckets"
        ...
        self.size = size
        empty_entry = Entry.get_empty(
            *self.internal_entry_size(), index_size=self.index_size
        )
        ...
        self.stash = TrivialORAM(
            stash_size, vt, entry_size=es, index_size=self.index_size
        )
    ...
    def prepare_deepest(self, path):
        ...
        level, _, index = find_deepest(
            self.stash.ram.get_value_array(0), path, 0, self.D
        )
        goal = if_else(
            stash_empty, ValueTuple([0] * len(level)), unary_to_binary(level)
        )
        ...
        for i in range(1, self.L):
            l, _, index = find_deepest(
                buckets[i].get_value_array(0), path, i - 1, self.D
            )
        ...
        def op(left, right, void=None):
            ...
            goal = if_else(replace, bit_compose(l), bit_compose(goal)).bit_decompose(
                len(goal)
            )
    ...
    def prepare_target(self, path, deepest):
        ...
        for i in range(self.L, -1, -1):
            ...
            if i == 0:
                break
            cond = or_op(
                dest.empty * self.get_bucket_ram(path, i).has_empty_entry(),
                (1 - target[i].empty),
            ) * (1 - deepest[i].empty)
    ...
    def evict(self):
        raise CompilerError(
            "Using this function is likely an error. Use recursive_evict() instead."
        )
        ...
        for i, _ in enumerate(self.evict_rounds()):
            Program.prog.curr_tape.start_new_basicblock(
                name="circuit-evict-round-%d-%d" % (i, self.size)
            )

    def recursive_evict(self):
        Program.prog.curr_tape.start_new_basicblock(
            name="circuit-recursive-evict-%d" % self.size
        )
        for i, _ in enumerate(self.recursive_evict_rounds()):
            Program.prog.curr_tape.start_new_basicblock(
                name="circuit-recursive-evict-round-%d-%d" % (i, self.size)
            )
