...
from mpspdz.program import Tape
from mpspdz.exceptions import *
from mpspdz.instructions import *
from mpspdz.instructions_base import *
...
class MPCThread(object):
    def __init__(self, target, name, args=[], runtime_arg=0, single_thread=False):
        """ Create a thread from a callable object. """
        if not callable(target):
            raise CompilerError(
                "Target %s for thread %s is not callable" % (target, name)
            )
        self.name = name
        ...
        self.running = 0
        self.tape_handle = program.new_tape(
            target, args, name, single_thread=single_thread
        )
        self.run_handles = []

    def start(self, runtime_arg=None):
        self.running += 1
        self.run_handles.append(
            program.run_tape(self.tape_handle, runtime_arg or self.runtime_arg)
        )
...
def vectorize(operation):
    def vectorized_operation(self, *args, **kwargs):
        if len(args):
            ...
            if (
                (isinstance(args[0], Tape.Register) or isinstance(args[0], sfloat))
                and not isinstance(args[0], bits)
                and args[0].size != self.size
            ):
                raise CompilerError(
                    "Different vector sizes of operands: %d/%d"
...
def vectorize_init(function):
    def vectorized_init(*args, **kwargs):
        size = None
        if len(args) > 1 and (
            isinstance(args[1], Tape.Register) or isinstance(args[1], sfloat)
        ):
            size = args[1].size
            if (
                "size" in kwargs
                and kwargs["size"] is not None
...
class _number(object):
    ...
    def dot_product(cls, a, b):
        from mpspdz.library import for_range_opt_multithread
...
class _structure(object):
    ...
    def row_matrix_mul(cls, row, matrix, res_params=None):
        return sum(
            row[k].mul_no_reduce(matrix[k].get_vector(), res_params)
...
class _register(Tape.Register, _number, _structure):
    ...
    def _load_mem(cls, address, direct_inst, indirect_inst):
        if isinstance(address, _register):
            ...
            res = cls(size=size)
            indirect_inst(res, cls._expand_address(address, get_global_vector_size()))
        else:
            res = cls()
...
class _secret(_register):
    ...
    def row_matrix_mul(cls, row, matrix, res_params=None):
        ...
        dotprods(
            *sum(
                (
                    [res[j], row, [matrix[k][j] for k in range(len(row))]]
                    for j in range(size)
                ),
                [],
            )
    ...
    @vectorize
    def load_other(self, val):
        from mpspdz.GC.types import sbits, sbitvec
    ...
    def mul(self, other):
        ...
        :param other: any compatible type """
        if (
            isinstance(other, _secret)
            and (1 in (self.size, other.size))
    ...
    def reveal_to(self, player):
        ...
        :param player: int
        :returns: value to be used with :py:func:`mpspdz.library.print_ln_to`
        """
...
class sint(_secret, _int):
    ...
    def get_random_int(cls, bits):
        ...
        if program.use_split() == 3:
            ...
            overflow = comparison.CarryOutLE(x[1][:-1], x[0][:-1]) + sint.conv(x[0][-1])
    ...
    def get_dabit(cls):
        ...
        from mpspdz.GC.types import sbits
    ...
    def __eq__(self, other, bit_length=None, security=None):
        return floatingpoint.EQZ(
            self - other, bit_length or program.bit_length, security or program.security
        )
    ...
    def pow2(self, bit_length=None, security=None):
        """ Secret power of two. """
        return floatingpoint.Pow2(
            self, bit_length or program.bit_length, security or program.security
        )
    ...
    def round(self, k, m, kappa=None, nearest=False, signed=False):
        ...
        if nearest:
            ...
            return comparison.TruncRoundNearest(self, k, m, kappa, signed=signed)
        else:
            if secret:
    ...
    def split_to_n_summands(self, length, n):
        ...
        columns = [
            [sbits.get_type(self.size)() for i in range(n)] for i in range(length)
        ]
    ...
    def reveal_to(self, player):
        ...
        if not util.is_constant(player) or self.size > 1:
            ...
            return personal(player, (self + secret_mask).reveal() - player_mask)
        else:
...
class sgf2n(_secret, _gf2n):
    ...
    def __and__(self, other):
        ...
        :param other: sg2fn/cgf2n/regint/int """
        if isinstance(other, int):
            other_bits = [(other >> i) & 1 for i in range(program.galois_length)]
        else:
            other_bits = other.bit_decompose()
        self_bits = self.bit_decompose()
        return sum((x * y) << i for i, (x, y) in enumerate(zip(self_bits, other_bits)))
    ...
    def bit_decompose(self, bit_length=None, step=1):
        ...
        if bit_length == 0:
            return []
        bit_length = bit_length or program.galois_length
        random_bits = [self.get_random_bit() for i in range(0, bit_length, step)]

        one = cgf2n(1)
        masked = sum(
            [b * (one << (i * step)) for i, b in enumerate(random_bits)], self
        ).reveal()
    ...
    def bit_decompose_embedding(self):
        random_bits = [self.get_random_bit() for i in range(8)]
        one = cgf2n(1)
        wanted_positions = [0, 5, 10, 15, 20, 25, 30, 35]
        masked = sum(
            [b * (one << wanted_positions[i]) for i, b in enumerate(random_bits)], self
        ).reveal()
        return [
            self.clear_type((masked >> wanted_positions[i]) & one) + r
            for i, r in enumerate(random_bits)
        ]
...
class _bitint(object):
    ...
    def bit_adder(cls, a, b, carry_in=0, get_carry=False):
        ...
        a += [0] * (len(b) - len(a))
        b += [0] * (len(a) - len(b))
        return cls.bit_adder_selection(a, b, carry_in=carry_in, get_carry=get_carry)

    @classmethod
    def bit_adder_selection(cls, a, b, carry_in=0, get_carry=False):
        if cls.log_rounds:
            return cls.carry_lookahead_adder(
                a, b, carry_in=carry_in, get_carry=get_carry
            )
        elif cls.linear_rounds:
            return cls.ripple_carry_adder(a, b, carry_in=carry_in, get_carry=get_carry)
        else:
            return cls.carry_select_adder(a, b, carry_in=carry_in, get_carry=get_carry)

    @classmethod
    def carry_lookahead_adder(cls, a, b, fewer_inv=False, carry_in=0, get_carry=False):
        lower = []
        a, b = a[:], b[:]
    ...
    def sum_from_carries(a, b, carries):
        return [ai.bit_xor(bi).bit_xor(carry) for (ai, bi, carry) in zip(a, b, carries)]
    ...
    def carry_select_adder(cls, a, b, get_carry=False, carry_in=0):
        ...
        if sum(blocks) != n:
            raise Exception("blocks not summing up: %s != %s" % (sum(blocks), n))
        res = []
        carry = carry_in
    ...
    def load_int(self, other):
        if -(2 ** (self.n_bits - 1)) <= other < 2 ** (self.n_bits - 1):
            self.bin_type.load_int(
                self, other + 2 ** self.n_bits if other < 0 else other
            )
        else:
            raise CompilerError(
                "Invalid signed %d-bit integer: %d" % (self.n_bits, other)
            )
    ...
    def wallace_tree_from_matrix(cls, bit_matrix, get_carry=True):
        columns = [
            [
                _f
                for _f in (
                    bit_matrix[j][i - j] for j in range(min(len(bit_matrix), i + 1))
                )
                if not is_zero(_f)
            ]
    ...
    def __sub__(self, other):
        ...
        b = util.bit_decompose(other, self.n_bits)
        d = [
            (reduce(util.bit_xor, (ai, bi, 1)), (1 - ai) * bi) for (ai, bi) in zip(a, b)
        ]
        borrow = lambda y, x, *args: (x[0] * y[0], 1 - (1 - x[1]) * (1 - x[0] * y[1]))
        borrows = (0,) + list(zip(*floatingpoint.PreOpL(borrow, d)))[1]
        return self.compose(
            reduce(util.bit_xor, (ai, bi, borrow))
...
class intbitint(_bitint, sint):
    ...
    @staticmethod
    def sum_from_carries(a, b, carries):
        return [a[i] + b[i] + carries[i] - 2 * carries[i + 1] for i in range(len(a))]
...
class sgf2nuint(sgf2nint):
    def load_int(self, other):
        if 0 <= other < 2 ** self.n_bits:
            ...
            raise CompilerError(
                "Invalid unsigned %d-bit integer: %d" % (self.n_bits, other)
            )
...
class cfix(_number, _structure):
    ...
    def print_plain(self):
        ...
        if self.k > 64:
            raise CompilerError(
                "Printing of fixed-point numbers not "
        ...
        print_float_plain(cint(abs_v), cint(-self.f), cint(0), cint(sign), cint(0))
...
class _single(_number, _structure):
    ...
    def receive_from_client(cls, n, client_id, message_type=ClientMessageType.NoType):
        ...
        sint_inputs = cls.int_type.receive_from_client(
            n, client_id, ClientMessageType.TripleShares
        )
    ...
    def unreduced_dot_product(cls, x, y, res_params=None):
        dp = cls.int_type.dot_product(
            [xx.pre_mul() for xx in x], [yy.pre_mul() for yy in y]
        )
    ...
    def row_matrix_mul(cls, row, matrix, res_params=None):
        ...
        col = cls.int_type.row_matrix_mul([x.pre_mul() for x in row], int_matrix)
        res = (
            row[0].unreduced(col, matrix[0][0], res_params, len(row)).reduce_after_mul()
        )
        return res
...
class _fix(_single):
    ...
    def mul(self, other):
        ...
        if isinstance(other, (_fix, self.clear_type)):
            val = self.v.TruncMul(
                other.v, self.k + other.k, other.f, self.kappa, self.round_nearest
            )
    ...
    def __truediv__(self, other):
        ...
        if isinstance(other, _fix):
            v = library.FPDiv(
                self.v, other.v, self.k, self.f, self.kappa, nearest=self.round_nearest
            )
        elif isinstance(other, cfix):
            v = library.sint_cint_division(self.v, other.v, self.k, self.f, self.kappa)
        else:
            raise TypeError("Incompatible fixed point types in division")
    ...
    def compute_reciprocal(self):
        ...
        return type(self)(
            library.FPDiv(cint(2) ** self.f, self.v, self.k, self.f, self.kappa, True)
        )
...
class sfix(_fix):
    ...
    def reveal_to(self, player):
        ...
        :returns: value to be used with :py:func:`mpspdz.library.print_ln_to`
        """
        return personal(player, cfix(self.v.reveal_to(player)._v, self.k, self.f))
...
class squant(_single):
    ...
    def mul_no_reduce(self, other, res_params=None):
        if isinstance(other, (sint, cint, regint)):
            return self._new(
                other * (self.v - self.Z) + self.Z, params=self.get_params()
            )
        other = self.coerce(other)
        tmp = (self.v - self.Z) * (other.v - other.Z)
        return _unreduced_squant(
            tmp, (self.get_params(), other.get_params()), res_params=res_params
        )
    ...
    def unreduced(self, v, other, res_params=None, n_summands=1):
        return _unreduced_squant(
            v, (self.get_params(), other.get_params()), res_params, n_summands
        )
...
class squant_params(object):
    ...
    def reduce(self, unreduced):
        ...
        if reduce(operator.and_, (p.is_constant() for p in ps)):
            n_shift, int_mult, shifted_Z = self.get(
                unreduced.params, unreduced.n_summands
            )
        else:
...
class sfloat(_number, _structure):
    ...
    def convert_float(v, vlen, plen):
        ...
        if v == 0:
            ...
            if p >= 2 ** (plen - 1):
                raise CompilerError(
                    "Cannot convert %s to float " "with %d exponent bits" % (vv, plen)
                )
        return v, p, z, s
    ...
    def __init__(self, v, p=None, z=None, s=None, size=None):
        ...
        if p is None:
            if isinstance(v, sfloat):
                ...
                f = v.f
                v, p, z, s = floatingpoint.Int2FL(v.v, v.k, self.vlen, self.kappa)
                p = p - f
            elif util.is_constant_float(v):
                v, p, z, s = self.convert_float(v, self.vlen, self.plen)
            else:
                v, p, z, s = floatingpoint.Int2FL(
                    sint.conv(v), program.bit_length, self.vlen, self.kappa
                )
        if isinstance(v, int):
        ...
        if isinstance(p, int):
            if not (p >= -(2 ** (self.plen - 1)) and p < 2 ** (self.plen - 1)):
                raise CompilerError(
                    "Floating point number malformed: exponent %d not unsigned %d-bit integer"
                    % (p, self.plen)
                )
            self.p = sint(p)
        else:
    ...
    def add(self, other):
        ...
        if isinstance(other, sfloat):
            ...
            s3 = s1 + s2 - 2 * s1 * s2
            comparison.LTZ(
                d, self.vlen + pmin - pmax + sfloat.round_nearest, self.plen, self.kappa
            )
            pow_delta = floatingpoint.Pow2(
                (1 - d) * (pmax - pmin),
            ...
            if program.options.ring:
                ...
                v = floatingpoint.TruncInRing(
                    to_trunc, 2 * (self.vlen + 1 + sfloat.round_nearest), pow_delta
                )
            else:
                ...
                comparison.Trunc(
                    t,
                    v,
                    2 * self.vlen + 1 + sfloat.round_nearest,
                    self.vlen - 1,
                    self.kappa,
                    False,
                )
                v = t
            u = floatingpoint.BitDec(
                v,
                ...
                list(
                    range(
                        1 + sfloat.round_nearest, self.vlen + 2 + sfloat.round_nearest
                    )
                ),
            )
            ...
            if self.round_nearest:
                t2, overflow = floatingpoint.TruncRoundNearestAdjustOverflow(
                    pow_p0 * v, self.vlen + 3, self.vlen, self.kappa
                )
                p0 = p0 - overflow
            else:
            ...
            s = (1 - b) * (a * other.s + aneg * self.s) + b * (
                c * other.s + cneg * self.s
            )
    ...
    def mul(self, other):
        ...
        if isinstance(other, sfloat):
            ...
            if sfloat.round_nearest:
                v1 = comparison.TruncRoundNearest(
                    self.v * other.v, 2 * self.vlen, self.vlen - 1, self.kappa
                )
            else:
                comparison.Trunc(
                    v1,
            ...
            z1, z2, s1, s2, p1, p2 = (
                x.expand_to_vector()
                for x in (self.z, other.z, self.s, other.s, self.p, other.p)
            )
    ...
    def __truediv__(self, other):
        ...
        :param other: sfloat/float/sfix/sint/cint/regint/int """
        other = self.conv(other)
        v = floatingpoint.SDiv(
            self.v,
            other.v + other.z * (2 ** self.vlen - 1),
            self.vlen,
            self.kappa,
            self.round_nearest,
        )
    ...
    def __lt__(self, other):
        ...
        if isinstance(other, sfloat):
            ...
            d = ((1 - 2 * self.s) * self.v).less_than(
                (1 - 2 * other.s) * other.v, self.vlen + 1, self.kappa
            )
            cd = c * d
            ca = c * a
            b1 = cd + a - ca
            ...
            b = (
                (z1 - z12) * (1 - s2)
                + (z2 - z12) * s1
                + (1 + z12 - z1 - z2) * (s1 - s12 + (1 + s12 - s1 - s2) * b1 + s12 * b2)
            )
            return b
        else:
            return NotImplemented
    ...
    def __eq__(self, other):
        ...
        return (
            floatingpoint.EQZ(self.v - other.v, self.vlen, self.kappa)
    ...
    def reveal(self):
        ...
        :return: cfloat """
        return cfloat(
            self.v.reveal(), self.p.reveal(), self.z.reveal(), self.s.reveal()
        )
...
class Array(object):
    ...
    def get_address(self, index):
        ...
        if (program.curr_block, key) not in self.address_cache:
            ...
            self.address_cache[program.curr_block, key] = util.untuplify(
                [base + i * length for i in range(n)]
            )
            if self.debug:
                library.print_ln_if(index >= self.length, "OF:" + self.debug)
                library.print_ln_if(
                    self.address_cache[program.curr_block, key]
                    >= program.allocated_mem[self.value_type.reg_type],
                    "AOF:" + self.debug,
                )
    ...
    def get(self, indices):
        return self.value_type.load_mem(
            regint.inc(len(indices), self.address, 0) + indices, size=len(indices)
        )
...
class SubMultiArray(object):
    ...
    def __init__(self, sizes, value_type, address, index, debug=None):
        ...
        if debug:
            library.print_ln_if(
                self.address
                + reduce(operator.mul, self.sizes) * self.value_type.n_elements()
                > program.allocated_mem[self.value_type.reg_type],
                "AOF%d:" % len(self.sizes) + self.debug,
            )
    ...
    def __getitem__(self, index):
        ...
        if key not in self.sub_cache:
            if self.debug:
                library.print_ln_if(
                    index >= self.sizes[0], "OF%d:" % len(self.sizes) + self.debug
                )
            if len(self.sizes) == 2:
                self.sub_cache[key] = Array(
                    self.sizes[1],
                    self.value_type,
                    self.address + index * self.sizes[1] * self.value_type.n_elements(),
                    debug=self.debug,
                )
            else:
                self.sub_cache[key] = SubMultiArray(
                    self.sizes[1:],
                    self.value_type,
                    self.address,
                    index,
    ...
    def get_part_vector(self, base=0, size=None):
        ...
        return self.value_type.load_mem(self.address + base * part_size, size=size)
    ...
    def input_from(self, player, budget=None, raw=False):
        ...
        if (self.total_size() < budget) and self.value_type.n_elements() == 1:
            if raw or program.always_raw():
                input_from = self.value_type.get_raw_input_from
            else:
                input_from = self.value_type.get_input_from
            ...
            @library.for_range_opt(self.sizes[0], budget=budget / self[0].total_size())
            def _(i):
        ...
        if isinstance(other, Array):
            ...
            try:
                ...
                res_matrix.assign_vector(
                    self.value_type.matrix_mul(A, B, self.sizes[1], res_params)
                )
    ...
    def budget_mul(self, other, n_rows, row, n_columns, column, reduce=True, res=None):
        assert len(self.sizes) == 2
        assert len(other.sizes) == 2
        if res is None:
            if reduce:
                ...
                res_matrix = Matrix(n_rows, n_columns, self.value_type.unreduced_type)
        else:
            res_matrix = res
    ...
    def mul_trans(self, other):
        ...
        return self.budget_mul(
            other, self.sizes[0], lambda x, i: x[i], other.sizes[0], lambda x, j: x[j]
        )
    ...
    def parallel_mul(self, other):
        ...
        for i in range(self.sizes[1]):
            ...
            incint(
                addresses, regint(self.address + i), self.sizes[1], other.sizes[1], n
            )
            a.append(self.value_type.load_mem(addresses, size=n))
            addresses = regint(size=n)
            incint(
                addresses,
                regint(other.address + i * other.sizes[1]),
                1,
                1,
                other.sizes[1],
            )
...
class MultiArray(SubMultiArray):
    ...
    def __init__(self, sizes, value_type, debug=None, address=None, alloc=True):
        ...
        if isinstance(address, Array):
            ...
            self.array = Array(
                reduce(operator.mul, sizes), value_type, address=address, alloc=alloc
            )
        SubMultiArray.__init__(
            self, sizes, value_type, self.array.address, 0, debug=debug
        )
        if len(sizes) < 2:
...
class Matrix(MultiArray):
    ...
    def __init__(self, rows, columns, value_type, debug=None, address=None):
        ...
        """
        MultiArray.__init__(
            self, [rows, columns], value_type, debug=debug, address=address
        )

    def set_column(self, index, vector):
        assert self.value_type.n_elements() == 1
        addresses = regint.inc(self.sizes[0], self.address + index, self.sizes[1])
        vector.store_in_mem(addresses)
...
class VectorArray(object):
    ...
    def __getitem__(self, index):
        return self.value_type.load_mem(
            self.array.address + index * self.vector_size, size=self.vector_size
        )
...
class MemValue(_mem):
    ...
    def write(self, value):
        ...
        if not isinstance(self.register, self.value_type):
            raise CompilerError(
                "Mismatch in register type, cannot write \
                %s to %s"
                % (type(self.register), self.value_type)
            )
    ...
    less_than = lambda self, other, bit_length=None, security=None: self.read().less_than(
        other, bit_length, security
    )
    greater_than = lambda self, other, bit_length=None, security=None: self.read().greater_than(
        other, bit_length, security
    )
    less_equal = lambda self, other, bit_length=None, security=None: self.read().less_equal(
        other, bit_length, security
    )
    greater_equal = lambda self, other, bit_length=None, security=None: self.read().greater_equal(
        other, bit_length, security
    )
    equal = lambda self, other, bit_length=None, security=None: self.read().equal(
        other, bit_length, security
    )
    not_equal = lambda self, other, bit_length=None, security=None: self.read().not_equal(
        other, bit_length, security
    )
    ...
    bit_decompose = lambda self, *args, **kwargs: self.read().bit_decompose(
        *args, **kwargs
    )
