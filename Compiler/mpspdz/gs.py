...
class OReverseMatrix(OMatrix):
    def __init__(self, N, M, oram_type=OptimalORAM, int_type=types.sint):
        ...
        self.M = M
        self.oram = oram_type(
            N, entry_size=(log2(N),) * M, init_rounds=0, value_type=int_type.basic_type
        )
...
class OStack:
    def __init__(self, N, oram_type=OptimalORAM, int_type=types.sint):
        ...
        self.oram = oram_type(
            N, entry_size=log2(N), init_rounds=0, value_type=int_type.basic_type
        )
...
class Matchmaker:
    def init_hard(self, n_loops=None):
        ...
        self.m_prefs = OMatrix(
            self.N, self.M, oram_type=self.oram_type, int_type=self.int_type
        )
        ...
        if self.reverse:
            self.f_ranks = OReverseMatrix(
                self.N, self.M, oram_type=self.oram_type, int_type=self.int_type
            )
        else:
            self.f_ranks = OMatrix(
                self.N, oram_type=self.oram_type, int_type=self.int_type
            )
        ...
        def f(i):
            ...
            def f(j):
                if self.reverse:
                    self.f_ranks[i] = tuple(
                        (-i - j + 2 * self.N - 2) % self.N for j in range(self.M)
                    )
                else:
    ...
    def init_easy(self):
        ...
        self.f_ranks = OMatrix(self.N, oram_type=self.oram_type, int_type=self.int_type)
    ...
    def propose(self, man, woman, for_real):
        ...
        print_str(
            "woman: %s, man: %s, fiance: %s, worst fiance: %s, ",
            *(x.reveal() for x in (woman, man, fiance, worst_fiance))
        )
        print_ln(
            "rank man: %s, rank fiance: %s, engaged: %s, leaving: %s",
            *(x.reveal() for x in (rank_man, rank_fiance, engaged, leaving))
        )
    ...
    def match(self, n_loops=None):
        ...
        self.wives = self.oram_type(
            self.N, entry_size=log2(self.N), init_rounds=0, value_type=self.basic_type
        )
        self.husbands = self.oram_type(
            self.N, entry_size=log2(self.N), init_rounds=0, value_type=self.basic_type
        )
        propose = self.oram_type(
            self.N, entry_size=log2(self.N), init_rounds=0, value_type=self.basic_type
        )
        self.unengaged = OStack(
            self.N, oram_type=self.oram_type, int_type=self.int_type
        )
    ...
    def __init__(
        self, N, M=None, reverse=False, oram_type=OptimalORAM, int_type=types.sint
    ):
        self.N = N
