from mpspdz.types import MemValue, read_mem_value, regint, Array, cint
from mpspdz.types import _bitint, _number, _fix, _structure, _bit, _vec, sint
from mpspdz.program import Tape, Program
from mpspdz.exceptions import *
from mpspdz import util, oram, floatingpoint, library
from mpspdz import instructions_base
import mpspdz.GC.instructions as inst
import operator
import math
...
class bits(Tape.Register, _structure, _bit):
    ...
    @staticmethod
    def PreOR(l):
        return [1 - x for x in floatingpoint.PreOpN(operator.mul, [1 - x for x in l])]
    ...
    def __init__(self, value=None, n=None, size=None):
        ...
        Tape.Register.__init__(self, self.reg_type, Program.prog.curr_tape, size=size)
    ...
    def load_other(self, other):
        if isinstance(other, cint):
            ...
            try:
                ...
                raise CompilerError(
                    "cannot convert %s/%s from %s to %s"
...
class cbits(bits):
    ...
    __add__ = lambda self, other: self.clear_op(
        other, inst.addcb, inst.addcbi, operator.add
    )
    __sub__ = lambda self, other: self.clear_op(
        -other, inst.addcb, inst.addcbi, operator.add
    )
    __xor__ = lambda self, other: self.clear_op(
        other, inst.xorcb, inst.xorcbi, operator.xor
    )
    __radd__ = __add__
    __rxor__ = __xor__
...
class sbits(bits):
    ...
    def load_int(self, value):
        if abs(value) > (1 << self.n):
            raise Exception("public value %d longer than %d bits" % (value, self.n))
        if self.n <= 32:
    ...
    def __and__(self, other):
        if util.is_zero(other):
            return 0
        elif util.is_all_ones(other, self.n) or (other is None and self.n == None):
            return self
    ...
    def xor_int(self, other):
        ...
        return self.bit_compose(
            [~x if y else x for x, y in zip(self_bits, other_bits)] + extra_bits
        )
    ...
    def __lshift__(self, i):
        return self.bit_compose(
            [sbit(0)] * i + self.bit_decompose()[: self.max_length - i]
        )
    ...
    def reveal(self):
        if self.n == None or self.n > max(self.max_length, self.clear_type.max_length):
...
class sbitvec(_vec):
        ...
        class sbitvecn(cls, _structure):
            ...
            def load_mem(cls, address):
                if not isinstance(address, int) and len(address) == n:
                    ...
                    return cls.from_vec(sbit.load_mem(address + i) for i in range(n))
    ...
    def coerce(self, other):
        if isinstance(other, cint):
            size = other.size
            return (
                other.get_vector(base, min(64, size - base))
    ...
    def if_else(self, x, y):
        assert len(self.v) == 1
        try:
            return self.from_vec(util.if_else(self.v[0], a, b) for a, b in zip(x, y))
        except:
...
class bitsBlock(oram.Block):
    ...
    def __init__(self, value, start, lengths, entries_per_block):
        ...
        self.entries = [
            sbits.bit_compose(self.value_bits[i * length :][:length])
...
class sbitint(_bitint, _number, sbits, _sbitintbase):
    ...
    def TruncMul(self, other, k, m, kappa=None, nearest=False):
        ...
        if len(self_bits) + len(other_bits) != k:
            raise Exception(
                "invalid parameters for TruncMul: "
                "self:%d, other:%d, k:%d" % (len(self_bits), len(other_bits), k)
            )
        t = self.get_type(k)
