...
from mpspdz.types import _secret
from mpspdz.exceptions import CompilerError
from mpspdz.library import *
from mpspdz.program import Program
from mpspdz import floatingpoint, comparison, permutation

from mpspdz.util import *
...
class intBlock(Block):
    ...
    def __init__(self, value, start, lengths, entries_per_block):
        ...
        if Program.prog.options.ring:
            ...
            self.lower, self.shift = floatingpoint.Trunc(
                self.value, self.n_bits, self.start, Program.prog.security, True
            )
...
class gf2nBlock(Block):
    ...
    def __init__(self, value, start, lengths, entries_per_block):
        ...
        Program.prog.curr_tape.start_new_basicblock(
            name="gf2n-block-init-%d" % entries_per_block
        )
        ...
        if entries_per_block == 2:
            ...
            self.bits = (
                list(map(operator.add, anti_bits[:length], prod_bits[length:]))
                + anti_bits[length:]
            )
            ...
            self.bits = [
                sum(map(operator.mul, choice_bits, value_bits[i::length]))
                for i in range(length)
            ]
            self.adjust = sum(bit << (i * length) for i, bit in enumerate(choice_bits))
        else:
            value_bits = bit_decompose(self.value, used_bits)
            l = log2(entries_per_block)
            start_bits = bit_decompose(start, l)
            ...
            selected = [
                power * bit + (1 - bit) for bit, power in zip(start_bits, powers)
            ]
            ...
            self.adjust = adjust
        Program.prog.curr_tape.start_new_basicblock(
            name="gf2n-block-init-end-%d" % entries_per_block
        )

    def set_slice(self, value):
        upper_bits = self.bits[sum(self.lengths) :]
        upper = (sum(b << i for i, b in enumerate(upper_bits)) * self.adjust) << sum(
            self.lengths
        )
        value = sum(v << start for v, start in zip(value, series(self.lengths)))
...
def demux_array(x, res=None):
    ...
    if n == 1:
        ...
        @for_range_multithread(
            get_n_threads(len(res)), max(1, n_parallel // len(b)), len(a)
        )
        def f(i):
...
class RefRAM(object):
    ...
    def __init__(self, index, oram):
        if debug_ram_size:
            ...
            def f():
                print_ln(
                    "invalid bucket index %s for %s buckets", index, oram.n_buckets()
                )
                crash()
        ...
        self.l = [
            oram.get_array(self.size, t, array.address + index * oram.bucket_size)
    ...
    def reveal(self):
        ...
        res = RAM(
            self.size,
            [t.clear_type for t in self.entry_type],
...
class AbstractORAM(object):
    ...
    def write(self, index, value):
        new_value = [
            ...
            for length, v in zip(
                self.entry_size, value if isinstance(value, (tuple, list)) else (value,)
            )
        ]
    ...
    def access(self, index, new_value, write, new_empty=False):
        return self._access(
            self.index_type.hard_conv(index),
            self.value_type.bit_type.hard_conv(write),
            self.value_type.bit_type.hard_conv(new_empty),
            *[
                self.value_type.get_type(length).hard_conv(v)
                for length, v in zip(self.entry_size, tuplify(new_value))
            ]
        )

    def read_and_maybe_remove(self, index):
        return self.read_and_remove(self.index_type.hard_conv(index)), self.state.read()

    @method_block
    def _read(self, index):
        return self.access(
            index, tuple(self.value_type.get_type(l)(0) for l in self.entry_size), False
        )
    ...
    def _access(self, index, write, new_empty, *new_value):
        Program.prog.curr_tape.start_new_basicblock(
            name="abstract-access-remove-%d" % self.size
        )
        ...
        if len(read_value) != self.value_length:
            raise Exception(
                "read_and_remove() of %s returns wrong length of "
                "read value: %d, should be %d"
                % (type(self), len(read_value), self.value_length)
            )
        Program.prog.curr_tape.start_new_basicblock(
            name="abstract-access-add-%d" % self.size
        )
        new_value = (
            ValueTuple(new_value)
        ...
        self.add(
            Entry(
                index,
                value,
                if_else(write, empty, read_empty),
    ...
    def delete(self, index, for_real=True):
        self.access(index, (self.value_type(0),) * self.value_length, for_real, True)
...
class RefTrivialORAM(EndRecursiveEviction):
    ...
    def empty_entry(self, apply_type=True):
        return Entry.get_empty(
            self.value_type, self.entry_size, apply_type, self.index_size
        )
    ...
    def search(self, read_index):
        if use_binary_search and self.value_type == sgf2n:
            ...
            if self.size >= 128:
                ...
                found = [
                    indices[j].equal(read_index, self.index_size) * (1 - empty_bits[j])
    ...
    def read_and_remove(self, read_index, skip=0):
        ...
        read_value = sum(
            (entry.x.skip(skip) for entry in prod_entries),
    ...
    def _read(self, index):
        ...
        read_value = sum(
            list(map(operator.mul, found, self.ram.get_values())),
    ...
    def _access(self, index, write, new_empty, *new_value):
        ...
        entries = [entry for entry in self.ram]
        prod_values = list(map(operator.mul, found, (entry.x for entry in entries)))
        ...
        for i, (entry, prod_value) in enumerate(zip(entries, prod_values)):
            ...
            delta_entry = Entry(
                access_here * (index - entry.v),
                access_here * (new_value - entry.x),
                found[i] - if_else(new_empty, 0, access_here),
            )
        ...
        if debug:
            ...
            for i, entry in enumerate(self.ram):
                if not entry.empty():
                    if entry.v in found:
                        raise Exception(
                            "found double %s in %s" % (str(entry.v), str(self.ram.l))
                        )
                    found.add(entry.v)
            if index is not None:
                for i, entry in enumerate(self.ram):
                    if not entry.empty() and index == entry.v:
                        raise Exception(
                            "not removed %s in %s" % (str(index), str(self.ram.l))
                        )
                ...
                def f():
                    ...
                    def f():
                        ...
                        if new_entry is not None:
                            ...
                            def f():
                                comps = Entry(x == y for x, y in zip(entry, new_entry))
    ...
    def binary_search(self, index):
        ...
        if self.contiguous and n <= 256:
            ...
            for i, x in enumerate(indices):
                indices[i] = sum(
                    y << (j * expand) for j, y in enumerate(x.bit_decompose(logn))
                )
            index = sum(
                y << (j * expand) for j, y in enumerate(index.bit_decompose(logn))
            )
        else:
            expand = 1
    ...
    def find_first_empty(self):
        prefix_empty = [0] + floatingpoint.PreOR(
            [empty for empty in self.ram.get_empty_bits()], Program.prog.security
        )
        return [prefix_empty[i + 1] - prefix_empty[i] for i in range(len(self.ram))]
    ...
    def add(self, new_entry, state=None, evict=None):
        ...
        if crash_on_overflow:
            ...
            def f():
                self.output()
                print_ln(
                    "New entry: %s:%s (empty: %s)",
                    new_entry.v.reveal(),
                    new_entry.x[0].reveal(),
                    new_entry.is_empty.reveal(),
                )
    ...
    def pop(self):
        ...
        empty_entry = self.empty_entry(False)
        prefix_empty = [0] + floatingpoint.PreOR(
            [1 - empty for empty in self.ram.get_empty_bits()], Program.prog.security
        )
        pop_here = [prefix_empty[i + 1] - prefix_empty[i] for i in range(len(self.ram))]
        entries = [entry for entry in self.ram]
            ...
            def f():
                for i, x in enumerate((entry.v,) + entry.x):
                    ...
                    def f():
                        ...
                        print_ln("%s " * len(pop_here), *(x.reveal() for x in pop_here))
                        crash()
    ...
    def batch_init(self, values):
        for i, value in enumerate(values):
            ...
            new_value = [
                MemValue(self.value_type.hard_conv(v))
...
class LinearORAM(TrivialORAM):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.index_vector = self.get_array(
            2 ** self.index_size, self.index_type.bit_type
        )
    ...
    def _read(self, index):
        maybe_start_timer(6)
        empty_entry = self.empty_entry(False)
        demux_array(bit_decompose(index, self.index_size), self.index_vector)

        @map_sum(
            get_n_threads(self.size),
            n_parallel,
            self.size,
            self.value_length + 1,
            [self.value_type.bit_type]
    ...
    def _write(self, index, *new_value):
        ...
        def f(i):
            ...
            delta_entry = Entry(
                0, access_here * (nv - entry.x), -access_here * entry.empty()
            )
    ...
    def _access(self, index, write, new_empty, *new_value):
        empty_entry = self.empty_entry(False)
        index_vector = demux_array(bit_decompose(index, self.index_size))
        new_value = make_array(new_value)
        ...
        @map_sum(
            ...
            self.size,
            self.value_length + 1,
            [self.value_type.bit_type] + [self.value_type] * self.value_length,
        )
        def f(i):
            entry = self.ram[i]
...
class RefBucket(object):
    ...
    def __repr__(self, depth=0):
        ...
        if depth < self.oram.D:
            result += self.ref_children(0).__repr__(depth + 1) + self.ref_children(
                1
            ).__repr__(depth + 1)
        return result
...
class List(EndRecursiveEviction):
    ...
    def __init__(
        self, size, value_type, value_length=1, init_rounds=None, entry_size=None
    ):
        self.value_type = value_type
        ...
        if entry_size is None:
            self.l = [
                value_type.dynamic_array(size, value_type) for i in range(value_length)
            ]
        else:
            self.l = [
                value_type.dynamic_array(size, value_type.get_type(length))
    ...
    __getitem__ = lambda self, index: [
        self.l[i][regint(reveal(index))] for i in range(self.value_length)
    ]
    ...
    add = lambda self, entry, **kwargs: self.__setitem__(
        entry.v.read(), [v.read() for v in entry.x]
    )
...
class LocalIndexStructure(List):
    ...
    def __init__(
        self, size, entry_size, value_type=sint, init_rounds=-1, random_init=False
    ):
...
class TreeORAM(AbstractORAM):
    ...
    def __init__(
        ...
        self.index_type = value_type.get_type(self.index_size)
        self.size = size
        empty_entry = Entry.get_empty(*self.internal_entry_size(), index_size=self.D)
        self.entry_type = empty_entry.types()
        self.ram = RAM(
            self.n_buckets() * self.bucket_size, self.entry_type, self.get_array
        )
        if init_rounds != -1:
            # put memory initialization in different timer
            stop_timer()
    ...
    def add_to_root(self, state, is_empty, v, *x):
        if len(x) != self.value_length:
            raise CompilerError(
                "value length mismatch: %s, should be %s" % (len(x), self.value_length)
            )
        l = state
    ...
    def evict_bucket(self, bucket, d):
        ...
        b = if_else(
            entry.empty(),
            self.value_type.bit_type.get_random_bit(),
        ...
        if debug_online:
            ...
            def f():
                ...
                bucket.ref_children(b).bucket.check(new_entry=secret_entry, op="evic")
                bucket.ref_children(1 - b).bucket.check(index=secret_entry.v, op="evic")
    ...
    def read_and_remove_levels(self, u, read_path):
        ...
        @map_sum(
            get_n_threads_for_tree(self.size),
            parallel,
            levels,
            self.value_length + 1,
            [self.value_type.bit_type] + [self.value_type] * self.value_length,
    ...
    def read_and_remove(self, u):
        ...
        #                              (str(res), str(u), str(l)))
        Program.prog.curr_tape.start_new_basicblock(
            name="read_and_remove-%d-end" % self.size
        )
    ...
    def batch_init(self, values):
        ...
        for i, index_value in enumerate(indexed_values):
            ...
            entries[i] = Entry(
                index,
                value,
                self.value_type.hard_conv(False),
                value_type=self.value_type,
            )
        ...
        for i in range(nleaves):
            ...
            if self.bucket_size + Program.prog.security > 128:
                ...
                bits2 = floatingpoint.B2U(
                    half + parity, half_max, Program.prog.security
                )[0]
                # (doesn't work)
                # bits2 = [0] * half_max
                ...
                bucket_bits = floatingpoint.B2U(
                    empty_positions[i] + 1, self.bucket_size, Program.prog.security
                )[0]
...
class PackedIndexStructure(object):
    ...
    def __init__(
        self, size, entry_size=None, value_type=sint, init_rounds=-1, random_init=False
    ):
        self.size = size
        if entry_size is None:
        ...
        for demux_bits in range(max_demux_bits + 1):
            self.log_entries_per_element = min(
                log2(size),
                int(
                    math.floor(
                        math.log(
                            float(get_value_size(value_type)) / sum(self.entry_size), 2
                        )
                    )
                ),
            )
            self.log_elements_per_block = max(
                0, min(demux_bits, log2(size) - self.log_entries_per_element)
            )
            if self.log_entries_per_element < 0:
                ...
                for s in self.entry_size:
                    if s > max_bits:
                        raise CompilerError(
                            "Inadequate entry size %d, " "maximum %d" % (s, max_bits)
                        )
        ...
        if real_size > 1:
            ...
            self.l = self.storage(
                real_size, value_type, entry_size=entry_size, init_rounds=0
            )
            self.small = False
        else:
            self.l = List(1, value_type, self.elements_per_block, entry_size=entry_size)
            self.small = True
        ...
        if init_rounds:
            ...
            def f(i):
                if random_init:
                    self.l[i] = [
                        random_block(self.used_bits, self.value_type)
    ...
    def translate_index(self, index):
        ...
        if self.value_type == sint:
            ...
            c = mod2m(
                rem, self.log_entries_per_element, self.log_entries_per_block, False
            )
            b = (rem - c).trunc_zeros(
                self.log_entries_per_element, self.log_entries_per_block
            )
            if self.small:
                return 0, b, c
            else:
                return (
                    (index - rem).trunc_zeros(
                        self.log_entries_per_block, log2(self.size)
                    ),
                    b,
                    c,
                )
    ...
    class Slicer(object):
        ...
        def read(self, block):
            self.block = block
            self.index_vector = demux(
                bit_decompose(self.b, self.pack.log_elements_per_block)
            )
            self.vector = list(map(operator.mul, self.index_vector, block))
            self.element = get_block(
                sum(self.vector),
                self.c,
    ...
    class MultiSlicer(object):
        ...
        def read(self, block):
            ...
            for element, sizes in zip(block, self.pack.split_sizes):
                ...
                for size in sizes:
                    res.append(sum(bit << i for i, bit in enumerate(bits[-size:])))
                    del bits[-size:]
            return tuple(res)
    ...
    def access(self, index, value, write, evict=True):
        ...
        value = if_else(write, ValueTuple(tuplify(value)), ValueTuple(read_value))
        self.l.add(
            Entry(
                MemValue(self.l.index_type(slicer.a)),
                ValueTuple(MemValue(v) for v in slicer.write(value)),
                value_type=self.value_type,
            ),
            evict=evict,
        )
...
class PackedORAMWithEmpty(AbstractORAM, PackedIndexStructure):
    def __init__(self, size, entry_size=None, value_type=sint, init_rounds=-1):
        ...
        PackedIndexStructure.__init__(
            self, size, (1,) + tuplify(entry_size), value_type, init_rounds=init_rounds
        )
    ...
    def access(self, index, new_value, write, new_empty=False, evict=True):
        res = PackedIndexStructure.access(
            self, index, (1 - new_empty,) + tuplify(new_value), write, evict=evict
        )
        return res[1:], 1 - res[0]
...
def oram_delete(oram, iterations=100):
    ...
    def f(i):
        x = oram.access(
            oram.value_type(i % oram.size),
            oram.value_type(0),
            oram.value_type(True),
            oram.value_type(True),
        )
