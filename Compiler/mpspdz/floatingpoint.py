...
def bits(a, m):
    ...
    if isinstance(a, int):
        ...
        res = []
        from mpspdz.types import regint, cint
...
def PreORC(a, kappa=None, m=None, raw=False):
    ...
    if k <= max_k:
        ...
        return sum(([or_op(x, y) for x in si] for si, y in zip(s[1:], t)), s[0])[-m:]
...
def B2U_from_Pow2(pow2a, l, kappa):
    ...
    if program.Program.prog.options.ring:
        ...
        asm_open(
            c, pow2a + two_power(l) * t + sum(two_power(i) * r[i] for i in range(l))
        )
...
def TruncRoundNearestAdjustOverflow(a, length, target_length, kappa):
    ...
    if program.Program.prog.options.ring:
        s = (1 - overflow) * t + comparison.TruncLeakyInRing(
            overflow * t, length, 1, False
        )
    else:
...
def TruncPrRing(a, k, m, signed=True):
    ...
    if k == n_ring:
        ...
        if program.Program.prog.use_trunc_pr:
            ...
            res = shifted - upper + (overflow << (k - m))
        if signed:
            res -= 1 << (k - m - 1)
        return res
...
def SDiv(a, b, l, kappa, round_nearest=False):
    ...
    for i in range(theta - 1):
        ...
        x = x1 * x1 + x.round(2 * l + 1, l - 1, kappa, round_nearest, signed=False)
        x2 = types.sint()
