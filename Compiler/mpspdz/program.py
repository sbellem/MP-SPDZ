...
from mpspdz.config import COST, REG_MAX, USER_MEM
from mpspdz.exceptions import CompilerError  # noqa F403
from mpspdz.instructions_base import RegType
import mpspdz.instructions
import mpspdz.instructions_base
import mpspdz.instructions_base as inst_base
...
data_types = dict(
    triple=0, square=1, bit=2, inverse=3, bittriple=4, bitgf2ntriple=5, dabit=6,
)

field_types = dict(modp=0, gf2n=1, bit=2,)
...
class Program:
    ...
    def __init__(self, args, options, *, source=None):
        ...
        self.to_merge = [
            mpspdz.instructions.asm_open_class,
            mpspdz.instructions.gasm_open_class,
            mpspdz.instructions.muls_class,
            mpspdz.instructions.gmuls_class,
            mpspdz.instructions.mulrs_class,
            mpspdz.instructions.gmulrs,
            mpspdz.instructions.dotprods_class,
            mpspdz.instructions.gdotprods_class,
            mpspdz.instructions.asm_input_class,
            mpspdz.instructions.gasm_input_class,
            mpspdz.instructions.inputfix_class,
            mpspdz.instructions.inputfloat_class,
            mpspdz.instructions.inputmixed_class,
            mpspdz.instructions.trunc_pr_class,
            mpspdz.instructions_base.Mergeable,
        ]
        import mpspdz.GC.instructions as gc
    ...
    def init_source(self, source):
        if not source:
            with open(self.infile) as source_file:
    ...
    def join_tape(self, thread_number):
        ...
        mpspdz.instructions.join_tape(thread_number)
    ...
    @staticmethod
    def build_sch_file_content(nonempty_tape_names, max_par_tapes, sys_argv):
        raise NotImplementedError

    def get_schedule(self):
        nonempty_tape_names = [t.name for t in self.tapes]
        sch_file_content = "\n".join(
            (
                str(self.max_par_tapes()),
                str(len(nonempty_tape_names)),
                " ".join(nonempty_tape_names),
                "1 0",
                "0",
                " ".join(sys.argv),
            )
        )
        return sch_file_content

    def write_bytes(self, outfile=None):
        """ Write all non-empty threads and schedule to files. """
        schedule = self.get_schedule()
        sch_filename = f"{self.programs_dir}/Schedules/{self.name}.sch"

        print("Writing to", sch_filename)
        with open(sch_filename, "w") as sch_file:
        ...
        for mem_type, size in list(self.allocated_mem.items()):
            if size:
                ...
                if mem_type in self.types:
                    ...
                    from mpspdz.types import _get_type
...
class Tape:
    ...
    class BasicBlock(object):
        ...
        def adjust_jump(self):
            ...
            # print(
            #     "Basic block %d jumps to %d (%d)"
            #     % (next_block_index, jump_index, offset)
            # )
        ...
        for req, num in sorted(self.req_num.items()):
            ...
            if req[1] in data_types:
                self.basicblocks[-1].instructions.append(
                    mpspdz.instructions.use(
                        field_types[req[0]], data_types[req[1]], num, add_to_prog=False
                    )
                )
            elif req[1] == "input":
                self.basicblocks[-1].instructions.append(
                    mpspdz.instructions.use_inp(
                        field_types[req[0]], req[2], num, add_to_prog=False
                    )
                )
            elif req[0] == "modp":
                self.basicblocks[-1].instructions.append(
                    mpspdz.instructions.use_prep(req[1], num, add_to_prog=False)
                )
            elif req[0] == "gf2n":
                self.basicblocks[-1].instructions.append(
                    mpspdz.instructions.guse_prep(req[1], num, add_to_prog=False)
                )
            elif req[0] == "edabit":
                self.basicblocks[-1].instructions.append(
                    mpspdz.instructions.use_edabit(
                        False, req[1], num, add_to_prog=False
                    )
                )
            elif req[0] == "sedabit":
                self.basicblocks[-1].instructions.append(
                    mpspdz.instructions.use_edabit(True, req[1], num, add_to_prog=False)
                )
        ...
        if not self.is_empty():
            ...
            for x in ("p", "2"):
                if self.req_bit_length[x]:
                    ...
                    self.basicblocks[-1].instructions.append(
                        mpspdz.instructions.reqbl(bl, add_to_prog=False)
                    )
            if self.program.verbose:
    ...
    class ReqNum(defaultdict):
        ...
        def pretty(self):
            t = lambda x: "integer" if x == "modp" else x  # noqa E731
            res = []
    ...
    class Register(object):
        ...
        def __init__(self, reg_type, program, size=None, i=None):
            ...
            if mpspdz.instructions_base.get_global_instruction_type() == "gf2n":
            ...
            self.program = program
            if size is None:
                size = mpspdz.instructions_base.get_global_vector_size()
