from mpspdz.oram import *
from mpspdz.oram import OptimalORAM
...
class HeapEntry(object):
    ...
    def __repr__(self):
        return (
            "("
            + ", ".join(
                "%s=%s" % (field, self.__dict__[field]) for field in self.fields
            )
            + ")"
        )
    ...
    def hard_conv_me(self, value_type):
        return type(self)(self.int_type, *(value_type.hard_conv(x) for x in self))
...
class HeapORAM(object):
    ...
    def add(self, index, entry, state):
        self.oram.add(
            Entry(MemValue(index), [MemValue(i) for i in entry.data()], entry.empty),
            state=state,
        )
...
class HeapQ(object):
    def __init__(self, max_size, oram_type=ORAM, init_rounds=-1, int_type=sint):
        ...
        self.value_index = oram_type(
            max_size,
            entry_size=log2(max_size),
            init_rounds=init_rounds,
    ...
    def _pop(self, for_real=True):
        Program.prog.curr_tape.start_new_basicblock(name="heapq-pop")
    ...
    def _update(self, value, prio, for_real=True):
        Program.prog.curr_tape.start_new_basicblock(name="heapq-update")
        ...
        self.value_index.access(value, self.basic_type(self.size), not_found * for_real)
    ...
    def check(self):
        if debug:
            for i in range(len(self.heap)):
                if (
                    (2 * i + 1 < len(self.heap) and self.heap[i] > self.heap[2 * i + 1])
                    or (
                        2 * i + 2 < len(self.heap)
                        and self.heap[i] > self.heap[2 * i + 2]
                    )
                ) and not self.heap[i].empty:
        ...
        if debug_online:
            ...
            def f(value):
                ...
                def f():
                    ...
                    def f():
                        print_ln(
                            "heap mismatch: %s:%s in index, %s in heap",
                            value,
                            index,
                            heap_value,
                        )
...
def dijkstra(source, edges, e_index, oram_type, n_loops=None, int_type=sint):
    ...
    dist = oram_type(
        e_index.size,
        entry_size=(32, log2(e_index.size)),
        init_rounds=vert_loops,
        value_type=basic_type,
    )
    ...
    def f(i):
        ...
        print_ln(
            "u: %s, v: %s, alt: %s, dv: %s, first visit: %s",
            u.reveal(),
            v.reveal(),
            alt.reveal(),
            dv[0].reveal(),
...
def test_dijkstra_on_cycle(n, oram_type=ORAM, n_loops=None, int_type=sint):
    ...
    edges = oram_type(
        n_edges,
        entry_size=(log2(n), log2(n), 1),
        init_rounds=0,
        value_type=int_type.basic_type,
    )
    e_index = oram_type(
        n, entry_size=log2(n_edges), init_rounds=0, value_type=int_type.basic_type
    )
...
def test_dijkstra_on_complete(n, oram_type=ORAM, n_loops=None, int_type=sint):
    ...
    if n_loops is not None:
        ...
        start_timer()
    return dijkstra(
        0, edges, e_index, oram_type, n_loops ** 2 if n_loops else n ** 2, int_type
    )
...
class ExtInt(object):
    ...
    def __lt__(self, other):
        if isinstance(other, ExtInt):
            return ((1 - self.inf) * (1 - other.inf) * (self.x < other.x)) + other.inf
        else:
            return (1 - self.inf) * (self.x < other)

    def __gt__(self, other):
        if isinstance(other, ExtInt):
            return ((1 - self.inf) * (1 - other.inf) * (self.x > other.x)) + self.inf
        else:
...
class VectorArray(Vector):
    def __init__(self, length, address=None):
        ...
        if address is None:
            ...
            self.arrays = [
                Array(length, "s", addr) for addr in (address, address + length)
            ]
...
def test_stupid_dijkstra(G, source):
    return stupid_dijkstra(
        convert_graph_to_matrix(G), demux(bit_decompose(source, log2(len(G))))
    )
