...
from mpspdz.util import tuplify, untuplify, is_zero
from mpspdz.allocator import RegintOptimizer
from mpspdz import instructions, instructions_base, comparison, program, util
import inspect, math
import random
import collections
import operator
    ...
    def __call__(self, *args):
        ...
        if len(args) not in self.type_args:
            ...
            def wrapped_function(*compile_args):
                base = get_arg()
                bases = dict(
                    (t, regint.load_mem(base + i))
                    for i, t in enumerate(sorted(type_args, key=lambda x: x.reg_type))
                )
        ...
        bases = dict((t, get_program().malloc(len(type_args[t]), t)) for t in type_args)
        for i, reg_type in enumerate(sorted(type_args, key=lambda x: x.reg_type)):
...
class FunctionBlock(Function):
    def on_first_call(self, wrapped_function):
        ...
        old_block.set_exit(
            instructions.jmp(0, add_to_prog=False), get_tape().active_basicblock
        )
                ...
                for i in range(len(a) // l):
                    for j in range(l // k):
                        ...
                        if k == 2:
                            ...
                            for m in range(
                                base + step, base + (k - 1) * step, 2 * step
                            ):
...
def chunkier_odd_even_merge_sort(
    a, n=None, max_chunk_size=512, n_threads=7, use_chunk_wraps=False
):
    if n is None:
        n = len(a)
        ...
        if size not in chunks:
            ...
            def swap_list(list_base):
                for i in range(size // 2):
                    base = list_base + 2 * i
                    x, y = cond_swap(sint.load_mem(base), sint.load_mem(base + 1))
                    store_in_mem(x, base)
    ...
    def run_setup(k, a_addr, step, tmp_addr):
        ...
        if not use_chunk_wraps or k <= 4:
            ...
            if k not in wrap_chunks:
                pre_chunk = FunctionTape(mem_op, "pre-%d" % k, compile_args=[True])
                post_chunk = FunctionTape(mem_op, "post-%d" % k, compile_args=[False])
...
def loopy_odd_even_merge_sort(a, sorted_length=1, n_parallel=32, n_threads=None):
    steps = {}
    l = sorted_length
    while l < len(a):
        l *= 2
    ...
    def merge(i_left, i_right, i_end):
        ...
        def loop(j):
            if_then(
                and_(
                    lambda: i0 < i_right,
                    or_(lambda: i1 >= i_end, lambda: regint(reveal(A[i0] <= A[i1]))),
                )
            )
            B[j] = A[i0]
            i0.iadd(1)
    ...
    def decorator(loop_body):
        ...
        if isinstance(n_parallel, int):
            if isinstance(n_loops, int):
                loop_rounds = n_loops // n_parallel if n_parallel < n_loops else 0
            else:
        ...
        if n_parallel is not None:
            ...
            def _(i):
                ...
                block = get_block()
                while (
                    (not util.is_constant(n_loops) or k < n_loops)
            ...
            if util.is_one(loop_rounds) and parent_block is blocks[-n_to_merge]:
                ...
                assert (
                    blocks[-n_to_merge + 1]
                    is get_tape().req_node.children[-1].nodes[0].blocks[0]
                )
...
def for_range_multithread(n_threads, n_parallel, n_loops, thread_mem_req={}):
    ...
    """
    return map_reduce(
        n_threads, n_parallel, n_loops, lambda *x: [], lambda *x: [], thread_mem_req
    )
...
def multithread(n_threads, n_items):
    ...
    """
    return map_reduce(
        n_threads, None, n_items, initializer=lambda: [], reducer=None, looping=False
    )
...
def map_reduce(
    ...
    if isinstance(n_loops, (list, tuple)):
        ...
        new_dec = map_reduce(
            n_threads, n_parallel, n_loops, initializer, reducer, thread_mem_req
        )
    ...
    def decorator(loop_body):
        ...
        def f(inc):
            ...
            if thread_mem_req:
                thread_mem = Array(
                    thread_mem_req[regint], regint, args[get_arg()].address + 2
                )
            mem_state = Array(
                len(state), type(state[0]) if state else cint, args[get_arg()][1]
            )

            @map_reduce_single(
                n_parallel, thread_rounds + inc, initializer, reducer, mem_state
            )
            def f(i):
                if thread_mem_req:
        ...
        if state:
            if thread_rounds:
                for i in range(n_threads - remainder):
                    state = reducer(
                        Array(len(state), type(state[0]), args[remainder + i][1]), state
                    )
            if remainder:
                for i in range(remainder):
                    state = reducer(
                        Array(len(state), type(state[0]).reg_type, args[i][1]), state
                    )
...
def do_while(loop_fn, g=None):
    ...
    get_tape().open_scope(lambda x: x[0].set_all(float("Inf")), name="begin-loop")
...
def if_then(condition):
    ...
    state.req_child = get_tape().open_scope(lambda x: x[0].max(x[1]), name="if-block")
...
def else_then():
    ...
    instructions.program.curr_tape.start_new_basicblock(
        state.start_block, name="else-block"
    )
...
def end_if():
    ...
    branch = instructions.jmpeqz(regint.conv(state.condition), 0, add_to_prog=False)
    # start next block
...
def IntDiv(a, b, k, kappa=None):
    return FPDiv(
        a.extend(2 * k) << k, b.extend(2 * k) << k, 2 * k, k, kappa, nearest=True
    )
