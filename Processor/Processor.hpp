...
template <class T>
SubProcessor<T>::SubProcessor(ArithmeticProcessor &Proc,
...
template <class T> SubProcessor<T>::~SubProcessor() {
...
Processor<sint, sgf2n>::Processor(int thread_num, Player &P,
  ...
  private_input_filename =
      (get_filename((machine.opts.prep_dir + "/Private-Input-").c_str(), true));
  private_input.open(private_input_filename.c_str());
  public_output.open(
      get_filename((machine.opts.prep_dir + "/Public-Output-").c_str(), true)
          .c_str(),
      ios_base::out);
  binary_output.open(get_parameterized_filename(
                         P.my_num(), thread_num,
                         (machine.opts.prep_dir + "/Binary-Output").c_str()),
                     ios_base::out);
...
template <class sint, class sgf2n> Processor<sint, sgf2n>::~Processor() {
...
template <class sint, class sgf2n>
string Processor<sint, sgf2n>::get_filename(const char *prefix,
                                            bool use_number) {
...
template <class sint, class sgf2n> void Processor<sint, sgf2n>::check() {
...
template <class sint, class sgf2n>
void Processor<sint, sgf2n>::edabit(const Instruction &instruction,
                                    bool strict) {
  auto &regs = instruction.get_start();
  int size = instruction.get_size();
  Procp.DataF.get_edabits(strict, size, &Procp.get_S_ref(instruction.get_r(0)),
                          Procb.S, regs);
}
...
void Processor<sint, sgf2n>::convcbitvec(const Instruction &instruction) {
  for (size_t i = 0; i < instruction.get_n(); i++) {
    ...
    Ci[instruction.get_r(0) + i] =
        Procb.C[instruction.get_r(1) + i1].get_bit(i2);
  }
}
...
// Write socket (typically SPDZ engine -> external client), for different
// register types. RegType and SecrecyType determines how registers are read and
// the socket stream is packed. If message_type is > 0, send message_type in
// bytes 0 - 3, to allow an external client to
//  determine the data structure being sent in a message.
template <class sint, class sgf2n>
void Processor<sint, sgf2n>::write_socket(const RegType reg_type,
                                          bool send_macs, int socket_id,
                                          int message_type,
                                          const vector<int> &registers,
                                          int size) {
  ...
  try {
    ...
    cerr << "Send error thrown when writing " << m << " values of type "
         << reg_type << " to socket id " << socket_id << "." << endl;
  }
}
...
template <class sint, class sgf2n>
void Processor<sint, sgf2n>::read_shares_from_file(
    int start_file_posn, int end_file_pos_register,
    const vector<int> &data_registers) {
  string filename;
  filename = "Persistence/Transactions-P" + to_string(P.my_num()) + "-" +
             to_string(P.N.get_portnum_base()) + ".data";
  ...
  try {
    binary_file_io.read_from_file(filename, outbuf, start_file_posn,
                                  end_file_posn, !machine.opts.no_persistence_file_sig);
...
// Append share data in data_registers to end of file. Expects Persistence
// directory to exist.
template <class sint, class sgf2n>
void Processor<sint, sgf2n>::write_shares_to_file(
    long start_pos, const vector<int> &data_registers) {
  string filename;
  filename = "Persistence/Transactions-P" + to_string(P.my_num()) + "-" +
             to_string(P.N.get_portnum_base()) + ".data";
  ...
  binary_file_io.write_to_file(filename, inpbuf, start_pos, !machine.opts.no_persistence_file_sig);
}
...
template <class T> void SubProcessor<T>::mulrs(const vector<int> &reg) {
  assert(reg.size() % 4 == 0);
  int n = reg.size() / 4;
...
void SubProcessor<T>::matmuls(const vector<T> &source,
                              const Instruction &instruction, size_t a,
                              size_t b) {
  ...
  for (int i = 0; i < dim[0]; i++)
    for (int j = 0; j < dim[2]; j++) {
      for (int k = 0; k < dim[1]; k++)
        protocol.prepare_dotprod(*(A + i * dim[1] + k), *(B + k * dim[2] + j));
      protocol.next_dotprod();
    }
  protocol.exchange();
  for (int i = 0; i < dim[0]; i++)
            ...
            for (int filter_x = 0; filter_x < weights_w; filter_x++) {
              ...
              if ((0 <= in_x) and (in_x < inputs_w)) {
                ...
                for (int in_c = 0; in_c < n_channels_in; in_c++)
                  protocol.prepare_dotprod(pixel_base[in_c], weight_base[in_c]);
    ...
    for (int out_y = 0; out_y < output_h; out_y++)
      for (int out_x = 0; out_x < output_w; out_x++) {
        output_base[out_y * output_w + out_x] =
            protocol.finalize_dotprod(lengths[i_batch][out_y][out_x]);
      }
  }
}
