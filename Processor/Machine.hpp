...
void Machine<sint, sgf2n>::init_binary_domains(int security_parameter,
                                               int lg2) {
  sgf2n::clear::init_field(lg2);

  if (not is_same<typename sgf2n::mac_key_type, GC::NoValue>()) {
    if (sgf2n::clear::degree() < security_parameter) {
      cerr << "Security parameter needs to be at most n in GF(2^n)." << endl;
      cerr << "Increase the latter (-lg2) or decrease the former (-S)." << endl;
      exit(1);
    }
  }
...
Machine<sint, sgf2n>::Machine(Names &playerNames, bool use_encryption,
    ...
    : my_number(playerNames.my_num()), N(playerNames), direct(opts.direct),
      opening_sum(opts.opening_sum), receive_threads(opts.receive_threads),
      max_broadcast(opts.max_broadcast), use_encryption(use_encryption),
  ...
  alphabi = read_generate_write_mac_key<typename sint::bit_type::part_type>(*P);
  ...
  if (memtype.compare("old") == 0) {
    ...
    if (inpf.fail()) {
      throw file_error(memory_filename());
    }
    inpf >> M2 >> Mp >> Mi;
    if (inpf.get() != 'M') {
...
void Machine<sint, sgf2n>::prepare(const string &progname_str) {
  ...
  for (auto &prog : progs) {
    if (prog.writes_persistence) {
      ...
      if (opts.no_persistence_file_sig) {
        if (pers.fail())
  ...
  if (live_prep and
      (sint::needs_ot or sgf2n::needs_ot or sint::bit_type::needs_ot)) {
...
template <class sint, class sgf2n> Machine<sint, sgf2n>::~Machine() {
...
template <class sint, class sgf2n>
void Machine<sint, sgf2n>::fill_buffers(
    int thread_number, int tape_number, Preprocessing<sint> *prep,
    Preprocessing<typename sint::bit_type> *bit_prep) {
  // central preprocessing
  auto usage = progs[tape_number].get_offline_data_used();
  if (sint::expensive and prep != 0 and
      OnlineOptions::singleton.bucket_size == 3) {
    try {
      auto &source = *dynamic_cast<BufferPrep<sint> *>(prep);
      auto &dest = dynamic_cast<BufferPrep<sint> &>(
          tinfo[thread_number].processor->DataF.DataFp);
      for (auto it = usage.edabits.begin(); it != usage.edabits.end(); it++) {
        bool strict = it->first.first;
        int n_bits = it->first.second;
        size_t required =
            DIV_CEIL(it->second, sint::bit_type::part_type::default_length);
        ...
        while (dest_buffer.size() < required) {
          ...
          size_t n = min(source_buffer.size(), required - dest_buffer.size());
          dest_buffer.insert(dest_buffer.end(), source_buffer.end() - n,
                             source_buffer.end());
          source_buffer.erase(source_buffer.end() - n, source_buffer.end());
        }
      }
    } catch (bad_cast &e) {
  ...
  if (bit_type::expensive_triples and bit_prep and
      OnlineOptions::singleton.bucket_size == 3) {
    try {
      auto &source = *dynamic_cast<BufferPrep<bit_type> *>(bit_prep);
      auto &dest = dynamic_cast<BufferPrep<bit_type> &>(
          tinfo[thread_number].processor->share_thread.DataF);
      for (int i = 0; i < DIV_CEIL(usage.files[DATA_GF2][DATA_TRIPLE],
                                   bit_type::default_length);
           i++)
        dest.push_triple(source.get_triple_no_count(bit_type::default_length));
    } catch (bad_cast &e) {
#ifdef VERBOSE_CENTRAL
      cerr << "Problem with central bit triple preprocessing: " << e.what()
           << endl;
#endif
    }
  }
    ...
    try {
      ...
      for (int j = 0; j < it->first[1]; j += max_inner) {
        for (int k = 0; k < it->first[2]; k += max_cols) {
          ...
          auto &source = dynamic_cast<Hemi<sint> &>(source_proc.protocol)
                             .get_matrix_prep(subdim, source_proc);
          auto &dest = dynamic_cast<Hemi<sint> &>(
                           tinfo[thread_number].processor->Procp.protocol)
                           .get_matrix_prep(
...
DataPositions Machine<sint, sgf2n>::run_tape(int thread_number, int tape_number,
  ...
  // printf("Send signal to run program %d in thread
  // %d\n",tape_number,thread_number); printf("Running line %d\n",exec);
  if (progs[tape_number].usage_unknown()) {
    if (not opts.live_prep and thread_number != 0) {
      insecure("Internally called tape " + to_string(tape_number) +
...
void Machine<sint, sgf2n>::run(const string &progname) {
  ...
  if (opts.verbose) {
    cerr << "Communication details "
            "(rounds in parallel threads counted double):"
         << endl;
    comm_stats.print();
  ...
  if (opening_sum < N.num_players() && !direct)
    cerr << "Summed at most " << opening_sum
         << " shares at once with indirect communication" << endl;
  else
  ...
  if (pos.any_more(progs[0].get_offline_data_used()) and
      not progs[0].usage_unknown())
...
template <class sint, class sgf2n> void Machine<sint, sgf2n>::reqbl(int n) {
  sint::clear::reqbl(n);
}
...
void Machine<sint, sgf2n>::suggest_optimizations() {
  ...
  if (relevant_opts.find("split") != string::npos and sint::has_split)
    optimizations.append("\tprogram.use_split(" + to_string(N.num_players()) +
                         ")\n");
